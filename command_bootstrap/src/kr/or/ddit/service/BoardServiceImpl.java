...
public class BoardServiceImpl<ReplyDAO> implements BoardService {
	...
	public BoardVO getBoard(int boardNo) throws SQLException {
		SqlSession session = sqlSessionFactory.openSession();
		try {
			BoardVO board = boardDAO.selectBoardbybno(session, boardNo);
			boardDAO.increaseViewCount(session, boardNo);
			return board;
		} finally {
			session.close();
		}
	...
	@Override
	public BoardVO getBoardForModify(int boardNo) throws SQLException {
		SqlSession session = sqlSessionFactory.openSession();
		try {
			BoardVO board = boardDAO.selectBoardbybno(session, boardNo);
			return board;
	...
	public void regist(BoardVO board) throws SQLException {
		SqlSession session = sqlSessionFactory.openSession();
		try {
			int boardNo = boardDAO.selectBoardbyboardNo(session);
	...
	public void remove(int boardNo) throws SQLException {
		SqlSession session = sqlSessionFactory.openSession();
		try {
			boardDAO.deleteBoard(session, boardNo);
		} finally {
			session.close();
		}
	}
